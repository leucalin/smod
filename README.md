...
### bilibili 登录
- 扫码登录页 `/bilibili-login`（不在侧栏展示，直接访问）
- 登录 cookie 保存在服务端，MV 搜索与取流会带上登录态，可降低部分接口的风控概率
- 登录接口：`/api/bilibili/login/qr`、`/login/check`、`/login/status`、`POST /api/bilibili/logout`
- 提示：bilibili 的风控主要依据 **IP 归属**，海外服务器登录后仍可能被拦截——登录能否免除风控请以实际测试为准
...
- 仓库：<https://github.com/leucalin/smod>
